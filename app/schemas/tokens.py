...
class TokenAuth(BaseModel):
    token_auth: str = Field("token_auth", title="Токен авторизации",
...
class TokenPair(BaseModel):
    token_long: str = Field("token_long", title="Длинный токен",
                            description="Действует  1год., необходим для получения нового короткого токена.")
    token_short: str = Field("token_short", title="Короткий токен",
